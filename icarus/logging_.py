...
class CacheStateLogger(BaseLogger):

    """
    Logging cache state for training data 
    """
    
    def __init__(self, file_name,caching_node_count):
        """
        Constructor
        """
        super(CacheStateLogger, self).__init__(file_name=file_name)
        col_names = ""
        for i in range(caching_node_count):
            for feat in ["node_memory", "dist_src", "dist_serving", "dist_rec", "lifetime_avg"]:
                col_names += feat+str(i)+"\t"

        col_names += "SourcePop\tPacketSize\tPacketLife\tHitType\tSevingNode\tPenalty\taction"
        print(len(col_names))
        self.append_push(col_names)
