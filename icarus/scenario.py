...
def req_generator(topology, n_contents, alpha, rate=12.0, duration_warmup=3000, duration_real=12000):
    ...
    receivers = [v for v in topology.nodes() if topology.node[v]['stack'][0] == 'receiver']
    zipf = ZipfDistribution(alpha, n_contents)
...
def scenario_geant(lifetime_range, size_range,net_cache=[0.05], n_contents=100000, alpha=[0.6, 0.8, 1.0]):
    """
    ...
    # for c in nx.connected_components(topology):
    #     print(c)
    #topology = (topology.subgraph(c) for c in nx.connected_components(topology))
    # print(topology)
    # print(type(topology))
    # i=1
    # for top in topology:
    ...
    deg = topology.degree()
    # degree_list = []
    # for sub_topology in topology:
    #     deg = nx.degree(sub_topology)
    #     print(sub_topology)
    ...
    for c in range(1, n_contents + 1):
        s = choice(sources)
        c_lifetime = randint(lifetime_range['MIN'], lifetime_range['MAX'])
        c_size = randint(size_range['MIN'], size_range['MAX'])
        c_record = (c,c_lifetime,c_size)
        contents[s].append(c_record)
    ...
    # topology.edge_color = edge_type

    for u, v in topology.edges():
        if u in sources or v in sources:
            topology[u][v]['type'] = 'external'
            # this prevents sources to be used to route traffic
            fnss.set_weights_constant(topology, 1000.0, [(u, v)])
            fnss.set_delays_constant(topology, external_link_delay, 'ms', [(u, v)])
        else:
            topology[u][v]['type'] = 'internal'
...
def neighbour_degree(graph, node):

    degree_sum=0
    deg = graph.degree()
    counter=0
    for neighbor in graph.neighbors(node):
        degree_sum+=deg[neighbor]
        counter+=1

    return degree_sum/counter

@register_scenario_generator('TISCALI')
def scenario_tiscali(lifetime_range, size_range, net_cache=[0.05], n_contents=100000, alpha=[0.6, 0.8, 1.0]):
    """
    ...
    # 240 nodes in the main component
    topology = fnss.parse_topology_zoo(path.join(scenarios_dir, 'resources/test.graphml')).to_undirected()
    #topology = list(nx.connected_component_subgraphs(topology))[0]
    ...
    # # they are 44
    # sources = [v for v in onedeg if neighbour_degree(topology,v) > 4.5] # they are  
    ...
    # # they are 36   
    # receivers = [v for v in onedeg if neighbour_degree(topology,v)  < 4.5]

    # # we set router stacks because some strategies will fail if no stacks
    # are deployed 


    sources = [v for v in topology.nodes() if topology.node[v]['label']=='source']
    receivers = [v for v in topology.nodes() if topology.node[v]['label']=='receiver']
    root = [v for v in topology.nodes() if topology.node[v]['label']=='root router']
    intermediate = [v for v in topology.nodes() if topology.node[v]['label']=='int router']
    edge = [v for v in topology.nodes() if topology.node[v]['label']=='edge router']

    caches = []
    caches.extend(root)
    caches.extend(intermediate)
    caches.extend(edge)
    ...
    for v in receivers:
        fnss.add_stack(topology, v, 'receiver', {})
    # for v in root:
    #     fnss.add_stack(topology, v, 'root', {})
    # for v in intermediate:
    #     fnss.add_stack(topology, v, 'intermediate', {})
    # for v in edge:
    ...
    # topology.edge_color = edge_type
    for nc in net_cache:
        size = (float(nc)*n_contents)*100/len(caches) # size of a single cache
        C = str(nc)
        for v in caches:
            fnss.add_stack(topology, v, 'cache', {'size': size})
        fnss.write_topology(topology, path.join(scenarios_dir, topo_prefix + 'T=%s@C=%s' % (T, C)  + '.xml'))
...
def scenario_wide(lifetime_range, size_range, net_cache=[0.05], n_contents=100000, alpha=[0.6, 0.8, 1.0]):
    """
    Return a scenario based on GARR topology
    ...
    for c in range(1, n_contents + 1):
        s = choice(sources)
        c_lifetime = randint(lifetime_range['MIN'], lifetime_range['MIN'])
        c_size = randint(size_range['MIN'], size_range['MAX'])
        c_record = (c,c_lifetime,c_size)
        contents[s].append(c_record)
...
def scenario_garr(lifetime_range, size_range, net_cache=[0.01, 0.05], n_contents=100000, alpha=[0.6, 0.8, 1.0]):
    """
    Return a scenario based on GARR topology
