...
from icarus.logging_ import EVENT_CACHE_HIT, EVENT_SERVER_HIT, \
...
class SummaryAnalyzer(object):
    ...
    def __init__(self, summary_file, scenario_params=['T', 'C', 'A', 'S']):
        ...
        Constructor
        """
        self.data = genfromtxt(summary_file, dtype=None, delimiter='\t', names=True,encoding=None)
        if scenario_params == None or len(scenario_params) == 0:
            return
        self.param = {}
        s = [SummaryAnalyzer.parse_scenario_id(str(s)) for s in self.data['Scenario']]
