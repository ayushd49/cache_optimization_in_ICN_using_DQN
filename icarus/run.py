...
import icarus.logging_ as logging_
...
def exec_experiment(topology, event_schedule, scenario_id, strategy_id, strategy_params=None):
    ...
    for time, event in event_schedule:
        strategy.handle_event(time, event,new_experiment)
...
def main():
    ...
    lifetime_range = config.CONTENT_LIFETIME
    size_range = config.CONTENT_SIZE
    ...
    if config.GEN_SCENARIOS:
        ...
        for t in topology_range:
            scenario_generator[t](lifetime_range, size_range,net_cache=net_cache_range, n_contents=n_contents, alpha=alpha_schedule)
    ...
    if calc_optimal_cache_hit:
        logging_.CacheHitRatioSummary(log_dir).append_optimal_cache_hit(topology_range, alpha_range, net_cache_range, n_contents)
